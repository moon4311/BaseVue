<template>
 ...
 export default defineComponent({
   setup() {
     ...
     const params = ref({});
     const columns = [
      {title:"시험ID",key:"EXAM_ID"},
      {title:"시험폴더ID",key:"EXAM_FOLDER_ID"},
      {title:"시험명",key:"EXAM_NAME"},
      {title:"문항수",key:"QUESTION_CNT"},
      {title:"평균난이도",key:"AVG_DIFFICULTY_LEVEL"},
      {title:"문제지상태",key:"EXAM_STATUS_CODE"},
        ...
        render(row : any) {
          return h(
            NButton,
            {
              ...
              tertiary: false,
              size: "small",
              onClick: () => util.showInfo(row.EXAM_ID)
            },
            { default: () => "Edit" }
          );
        }
     ...
     const search = ()=>{
       axios.get(apiUrl + "/dev/TB_EXAM/list",{params: params.value } ).then((res: any)=>{
